...
dfx deploy --network $IC_NETWORK --mode reinstall arcmindai_controller --argument "(opt principal \"$OwnerPrincipal\", opt principal \"$BrainPrincipal\", opt principal \"$ToolsPrincipal\", opt \"$BROWSE_WEBSITE_GPT_MODEL\")"
